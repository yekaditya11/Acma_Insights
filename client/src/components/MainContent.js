  ...
  const onDrop = async (acceptedFiles) => {
    ...
    try {
      ...
      if (response["general-insights"]) {
        ...
        if (Array.isArray(rawGeneralInsights)) {
          rawGeneralInsights.forEach(insight => {
            // Handle both old string format and new object format
            const insightText = typeof insight === 'object' ? insight.text : insight;
            summaryLines.push(`${pointCounter}. ${insightText}`);
            pointCounter++;
          });
        }
  ...
  const generateMoreInsights = async () => {
    ...
    try {
      ...
      if (response.additional_insights) {
        // Store both formatted text and raw objects for sentiment display
        const formattedInsights = response.additional_insights.map((insight, index) => {
          const insightText = typeof insight === 'object' ? insight.text : insight;
          return `${additionalInsights.length * 5 + index + 1}. ${insightText}`;
        }).join('\n');
        ...
        setAdditionalInsights((prevInsights) => [
          ...prevInsights,
          {
            ...
            rawInsights: response.additional_insights, // Store raw objects for sentiment
                      ...
                      {summary
                        ...
                        .map((line, index, array) => {
                          ...
                          if (match) {
                            ...
                            // Get the raw insight object if available for sentiment display
                            const rawInsight = tables.rawGeneralInsights && 
                              tables.rawGeneralInsights[parseInt(match[1]) - 1];
                            
                            return (
                              <InsightPoint
                                key={pointId}
                                pointId={pointId}
                                content={rawInsight || line}
                            ...
                            {insightSet.content
                              ...
                              .map((line, index, lineArray) => {
                                ...
                                if (match) {
                                  ...
                                  // Get the raw insight object if available for sentiment display
                                  const rawInsight = insightSet.rawInsights && 
                                    insightSet.rawInsights[index];
